...
import 'package:testpayment/widgets/bottom_baroptions.dart';
import 'package:testpayment/widgets/fab_options.dart';
...
class DetailScreen extends StatefulWidget {
  ...
  DetailScreen({Key key, @required this.loggedInUser, this.user}) : super(key: key);
...
class ArticlesScreen extends State<DetailScreen> {
  ...
  void initState() {
    ...
    StripePayment.setOptions(
        StripeOptions(publishableKey: "pk_test_gw4FRJIgo7hsOilkaQDTbPjo00tayhyI0B", merchantId: "Test", androidPayMode: 'test'));
  }
          ...
          return Column(
            ...
            children: <Widget>[
              ...
              Expanded(
                ...
                itemBuilder: (context, index) => buildBody(context, __.data.documents[index]),
              )),
              // Options()
            ],
          );
                ...
                IconButton(
                  ...
                  onPressed: () {
                    fInstance.runTransaction((transaction) async {
                      DocumentSnapshot snapshot = await transaction.get(docSnapshot.reference);

                      await transaction.update(snapshot.reference, {'dislike': snapshot['dislike'] + 1});
                    });
                  },
                ),
                IconButton(
