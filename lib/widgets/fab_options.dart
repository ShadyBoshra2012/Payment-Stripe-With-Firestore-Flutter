...
import 'package:testpayment/classes/PaymentItem.dart';
import 'package:testpayment/responses/charges_resp.dart';
import 'package:testpayment/utilities/CommonFunctions.dart';
import 'package:testpayment/utilities/CurrentUser.dart';
import 'package:testpayment/widgets/payment_list.dart';
...
class _FABOptionsState extends State<FABwithOptions> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    //Tweening the color.....
    _animateColor = ColorTween(begin: Colors.green, end: Colors.blue)
        .animate(CurvedAnimation(parent: _animationController, curve: Curves.easeIn, reverseCurve: Curves.easeOut));

    _animateFABButton = Tween<double>(begin: _fabHeight, end: -14.0).animate(CurvedAnimation(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Transform(
          transform: Matrix4.translationValues(0.0, _animateFABButton.value * 2.0, 0.0),
          child: addCard(),
        ),
        Transform(
          transform: Matrix4.translationValues(0.0, _animateFABButton.value, 0.0),
          child: payments(),
        ),
        fab()
      ],
  ...
  animation() {
    ...
    isFABOpened == true ? _animationController.forward() : _animationController.reverse();
  }
  ...
  Future<ChargesJSON> _getPayments() async {
    ...
    final uri = Uri.https('us-central1-test-payment-stripe.cloudfunctions.net', 'listCharges', {"email": "$email"});
  ...
  Future<String> _getSelectedPayment(ChargesJSON data) {
    return Navigator.push(
      ...
      MaterialPageRoute(
        ...
        builder: (context) =>
            Theme(data: _kTheme.copyWith(platform: Theme.of(context).platform), child: PaymentsList(convertListToWidget(data))),
      ),
    );
  }
}
